            ...
            <div class="flex flex-col md:flex-row items-center md:items-start gap-8">
                ...
                <div class="w-full md:w-1/2">
                    ...
                    <form action="{{ route('purchase.confirm') }}" method="POST" class="space-y-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label for="customer_name" class="block text-gray-700 font-semibold mb-2">氏名</label>
                                <input type="text" name="customer_name" id="customer_name" value="{{ $queryData['customer_name'] ?? '' }}" class="w-full border border-gray-300 rounded-lg px-3 py-2" placeholder="例) 鈴木一郎" required>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <div class="flex space-x-2">
                                    <input type="text" name="zipcode" id="zipcode" value="{{ $queryData['zipcode'] ?? '' }}" class="w-4/5 border border-gray-300 rounded-lg px-3 py-2" placeholder="例)ハイフン無しで1234567(-)" required>
                                    <button type="button" id="zipcodeSearch" class="w-1/5 bg-blue-500 text-white py-2 rounded-lg">入力</button>
                                </div>
                            </div>
                            <div class="mt-4">
                                <label for="city" class="block text-gray-700 font-semibold mb-2">都道府県</label>
                                <input type="text" name="city" id="city" value="{{ $queryData['city'] ?? '' }}" class="w-full border border-gray-300 rounded-lg px-3 py-2" placeholder="自動的に反映されます。" readonly>
                            </div>
                            <div class="mt-4">
                                <label for="detail_address" class="block text-gray-700 font-semibold mb-2">詳細住所</label>
                                <textarea name="detail_address" id="detail_address" cols="3" class="w-full border border-gray-300 rounded-lg px-3 py-2" placeholder="住所、建物名などを入力をお願いします。" required>{{ $queryData['detail_address'] ?? '' }}</textarea>
                            </div>
                        </div>
                        <!-- 사이즈 선택 -->
                        <div>
                            <label class="block text-gray-700 font-semibold mb-2">サイズを選択</label>
                        ...
                        <div class="w-3/5 mb-6">
                            <label for="quantity" class="block text-gray-700 font-semibold mb-2">数量</label>
