    ...
    <div class="container mx-auto flex justify-between items-center">
        ...
        <nav>
            <div class="flex justify-end space-x-4">
                @auth
                    <form method="GET">
                        <button type="submit" class="text-xs text-sky-700 underline" formaction="{{ route('user.mypage') }}" formmethod="get">
                            {{ Auth::user()->name }}
                        </button>
                    </form>
                    <form action="{{ url('/cart_index') }}" method="GET" style="display: inline;">
                        ...
                        <button type="submit" class="text-xs">
                            <div class="flex">
                                ...
                                <span id="cart-count" class="cart-count mt-1">{{ $cartCount ?? '' }}</span>
                            </div>
                        </button>
                    </form>
